...
import aqualert.fragments.BottomSheetFragment
import aqualert.helpers.AlarmHelper
import aqualert.helpers.SqliteHelper
import aqualert.utils.AppUtils
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onStart() {
            ...
            if (selectedOption != null) {
                if ((inTook * 100 / totalIntake) <= 140) {
                    if (sqliteHelper.addIntook(dateNow, selectedOption!!) > 0) {
                        ...
                        Snackbar.make(it, "Asupan air anda telah ditambahkan...!!", Snackbar.LENGTH_SHORT)
                            .show()

                    }
                } else {
                    Snackbar.make(it, "Anda telah memenuhi target!", Snackbar.LENGTH_SHORT).show()
                }
                selectedOption = null
                ...
                YoYo.with(Techniques.Shake)
                    .duration(700)
                    .playOn(cardView)
                Snackbar.make(it, "Silakan pilih salah satu opsi", Snackbar.LENGTH_SHORT).show()
            }
        }
        ...
        btnNotific.setOnClickListener {
            ...
            if (notificStatus) {
                btnNotific.setImageDrawable(getDrawable(R.drawable.ic_bell))
                Snackbar.make(it, "Notifikasi diaktifkan..", Snackbar.LENGTH_SHORT).show()
                alarm.setAlarm(
                    this,
                    sharedPref.getInt(AppUtils.NOTIFICATION_FREQUENCY_KEY, 30).toLong()
                )
                ...
                btnNotific.setImageDrawable(getDrawable(R.drawable.ic_bell_disabled))
                Snackbar.make(it, "Notifikasi dimatikan..", Snackbar.LENGTH_SHORT).show()
    ...
    private fun setWaterLevel(inTook: Int, totalIntake: Int) {
        ...
        if ((inTook * 100 / totalIntake) > 140) {
            Snackbar.make(main_activity_parent, "Anda telah mencapai target", Snackbar.LENGTH_SHORT)
                .show()
        }
    }
    ...
    override fun onBackPressed() {
        ...
        Snackbar.make(
            ...
            "Silakan tekan lagi untuk keluar",
